...
class DensityView {
    constructor(con, width, height) {
        ...
        //aaa is a placeholder since I want to leave the current references to data intact for now
        d3.csv('./games.csv')
            .then(aaa => {
                var victory_status_col = aaa.map(function(d) 
                    {return d.victory_status})
                var avg_rating_col = aaa.map(function(d) 
